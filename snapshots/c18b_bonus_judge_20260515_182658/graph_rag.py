...
class GraphRAGPipeline(Pipeline):
    ...
    async def run(self, query: str, **kwargs: Any) -> PipelineResult:
        ...
        # Adaptive fallback: if the community-mode answer is a refusal, retry
        # with cheap hybrid+chunk_only retrieval. Sum tokens honestly across
        # both calls.
        if not _is_refusal(primary.answer):
            if trim:
                primary.answer = _trim_answer(primary.answer)
            return primary

        logger.info("adaptive_fallback: refusal detected, retrying with hybrid+combine (C2 config)")
        ...
        # First-tier fallback: lean hybrid (chunk_only, top_k=1, num_hops=0).
        # This is the cheapest retrieval that bypasses both score_candidate
        # AND multi-hop traversal — typically ~1.0-1.5K tok / 1 LLM call. Tested:
        # finds the missing entities (Dario Amodei, Ilya Sutskever, Demis
        # Hassabis Nobel) that the community-summary primary missed.
        fb_kwargs.update({
            "method": "hybrid",
            "combine": True,
            "chunk_only": True,
        ...
        try:
            fallback = await self._run_once(query, **fb_kwargs)
        except Exception as e:
            logger.warning("adaptive_fallback (lean hybrid) failed (%s); trying full C2", e)
            # Second-tier: full C2 config (no chunk_only, more context).
            fb_kwargs.update({
                "chunk_only": False,
                "num_hops": 2,
            })
            try:
                ...
                logger.warning("adaptive_fallback (full C2) also failed (%s); returning primary", e2)
                return primary
